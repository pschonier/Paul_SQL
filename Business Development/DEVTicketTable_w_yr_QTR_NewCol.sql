...
CREATE VIEW vw_BusDevTickets
AS 

with products       ([ProductCode] ,
      [Name]) as
    (
...
SELECT  Shipper ,
		...
		d.Desig,
        origin ,
		ISNULL(mOrg.Market, '-') AS OriginMarket,
        MeterName AS Meter ,
        Location ,
		ISNULL(mLoc.Market, '-') AS DestMarket,
        netVol ,
        custody ,
        ReceiptDelivery ,
        ConnectionCode ,
		...
		DATENAME(MONTH, DATEADD(SECOND, endTime, '1970-1-1')) AS [Month]
...
WHERE   DATEPART(YEAR,GETDATE())  - DATEPART(YEAR,DATEADD(SECOND, endTime, '1970-1-1'))  <=2
